...
% Last Modified by GUIDE v2.5 15-Oct-2014 11:16:04
...
function LoadCSV(filename,pathname,handles)
...
function LoadNeuronCSV(filename,pathname,handles)
handles = guidata(handles.figure1);
if (~isempty(filename) && ~isempty(pathname))
    neuronHandler = CSVCoordinates();
    filepath = strcat(pathname,filename);
...
function LoadImageFolder(foldername,handles)
handles = guidata(handles.figure1);
  ...
  maxFilterList = zeros(0);
        ...
        while(i<maxIndex)
            ...
            while(y==previousY && i<=maxIndex)
             ...
             imageString = strcat(foldername, '/', filenamec3);
              if(exist(imageString,'file') && ~isempty(filenamec3))
              ...
              maxValueC3 = uint16(max(currentC3Pic(:)));
              maxValueWhole = uint16(max(maxValueNucleus,maxValueNeurite));
              maxValueWholeImage = uint16(max(uint16(maxValueWholeImage),maxValueWhole,maxValueC3));
          ...
          %    end
              ...
              c3ImageLine = uint16(cat(2,c3ImageLine,currentC3Pic));
              i = i + 1;
              if(i<=maxIndex)
                  x = SortedKeyArray(i,2);
                  y = SortedKeyArray(i,3);
    ...
    if(get(handles.rb4816,'Value') || get(handles.rb9616,'Value') || get(handles.rbWholeOt16,'Value')) %|| get(handles.rbOtSingleChamber16,'Value'))
        ...
        nucleusImage = flipdim(nucleusImage,1);
            ...
            filepath = [subfoldername '/' currentWell 'NeuriteBig' optionHandler.FileEnding];
            ...
            filepath = [subfoldername '/' currentWell 'Channel3Big' optionHandler.FileEnding];
            imwrite(c3Image,filepath);
            
            filepath = [subfoldername '/' currentWell 'NeuriteSmall' optionHandler.FileEnding];
...
foldername = uigetdir;
LoadImageFolder(foldername,handles);
...
function ConvertPicturesTo8Bit(handles)
handles = guidata(handles.figure1);
imageHandler = handles.ImageHandler;
optionHandler = handles.OptionHandler;
foldername = [imageHandler.Foldername '/ConvertedCellomics'];
...
if(cbRemoveCoreChecked)
    ...
    foldername = imageHandler.Foldername;
    [sizeY sizeX] = size(imageHandler.NeuriteImage);
    imMap = CutOutCircles(imMap,selectedWell,1, 0,0,optionHandler, foldername, sizeY, sizeX, NucleusM, NeuronM, imageHandler);
...
optionHandler.ToleranceAngleFromEndpoint = str2num(answer{11});
optionHandler.MaxDistanceFromEndpoint = str2num(answer{12});
optionHandler.MinNumberStrongThresholdPixels = str2num(answer{13});
optionHandler.MinDistanceBetweenNeurons = str2num(answer{14});
optionHandler.MinSizeNeuriteArea = str2num(answer{15});
optionHandler.FuzzFilterActivated = str2num(answer{16});
handles.OptionHandler = optionHandler;
guidata(handles.figure1, handles)
...
def = {num2str(optionHandler.HistogramMaxNucleus),num2str(optionHandler.HistogramMaxNeurite),num2str(optionHandler.HistogramMinNucleus),num2str(optionHandler.HistogramMinNeurite),num2str(optionHandler.MigDistLowerNucleusThreshold),num2str(optionHandler.MigDistLowerDensityThreshold), num2str(optionHandler.MigDistLowerFloodFillThreshold), num2str(optionHandler.DensityDistributionRingNumber),num2str(optionHandler.MigrationDistanceDensityImageXSize), num2str(optionHandler.MigrationDistanceDensityImageYSize),optionHandler.FileEnding,num2str(optionHandler.StartWell),optionHandler.ExcludedWells};
answer = inputdlg(prompt,dlg_title,num_lines,def);
...
optionHandler.FileEnding = answer{11};
optionHandler.StartWell = str2num(answer{12});
optionHandler.ExcludedWells = answer{13};
handles.OptionHandler = optionHandler;
guidata(handles.figure1, handles);
...
for i=1:numel(NonZeroY)    
  ...
  %      stack = java.util.Stack();
        ...
        if(startY-50>0 && startX-50>0 && startX+50 < sizeX && startY + 50 < sizeY)
            ...
            %Extract area
            numberNeuritePixels = bwarea(SubLabelComps == conCompIndex);
          ...
          %  [numberNeuritePixels, numberNucleusPixels] = FloodFillForEdgeFill(SubNucleusPic,SubArea,maxX,maxY,stack);
            ...
            %Extract area
            numberNeuritePixels = bwarea(LabelComps == conCompIndex);
        ...
        if(numberNeuritePixels/numberNucleusPixels >= area || numberNucleusPixels > 5000) 
            EdgeFillNeuronPositionList(curY, curX) = 1;
            EdgeFillNeuronPositionListSecond(curY, curX) = 1;
            edgeFillMarked=1;
        ...
        if(NeuronManualM(uint16(yStartPos)-1+uint16(curY),uint16(xStartPos)-1+uint16(curX)))
            manualMarked=1;            
            if(edgeFillMarked)
                TP = TP + 1;
                TN = TN - 1;
        ...
        if(save)
            ...
            key = mapPositionToKeyString(curY,curX);
...
if(numel(neuronHandler.OmniNucleusPositions) ==0 || ~isKey(neuronHandler.OmniNucleusPositions, selectedWell))
    neuronHandler.OmniNucleusPositions = containers.Map();
    neuronHandler.OmniNucleusPositions(selectedWell) = NucleusOM;
else
    fullMatrix = neuronHandler.OmniNucleusPositions(selectedWell);
    fullMatrix(yStartPos:yEndPos,xStartPos:xEndPos) = NucleusOM;
    neuronHandler.OmniNucleusPositions(selectedWell) = fullMatrix;
end
...
secondThreshold = 20;
% [bwYSize bwXSize] = size(BW);
% [bwstrongYSize bwstrongXSize] = size(BWStrong);
% if(bwYSize > bwstrongYSize)
%     BWStrong = [BWStrong;zeros(bwYSize-bwstrongYSize,bwstrongXSize)];
% end
% if(bwXSize > bwstrongXSize)
%     BWStrong = [BWStrong zeros(bwstrongYSize,bwXSize-bwstrongXSize)];
% end   
...
%         %FuzzTest Cut off evtl.
         if (optionHandler.FuzzFilterActivated > 0.5 && ((BrightDensity(yDens2,xDens2) < 3 && BrightDensity(yDens2+1,xDens2) < 3 && BrightDensity(yDens2+1,xDens2+1) < 3 && BrightDensity(yDens2,xDens2+1) < 3 && BrightDensity(yDens,xDens) < 3) || LowDensity(yDens,xDens) > 2000 || LowDensity(yDens+1,xDens+1) > 2000 || LowDensity(yDens+1,xDens) > 2000 || LowDensity(yDens,xDens+1) > 2000))
             fuzzy=1;
         end
     else
        noNeurite=0;
    ...
    innerMask=0;
     ...
     for j=1:numel(yPoses)
         ...
         if(isKey(currentStatDict,key))
           ...
           exportText = [exportText selectedWell ';' num2str(xPoses(j)) ';' num2str(yPoses(j)) ';' num2str(list(1)) ';' num2str(list(2)) ';' num2str(list(3)) ';' num2str(list(4)) ';' num2str(list(5)) ';' num2str(list(6)) ';' num2str(list(7)) ';' num2str(list(8)) ';' strrep(num2str(list(9)),'.',',') ';' strrep(num2str(list(10)),'.',',') ';' sprintf('\r\n')];
...
%Threshold
[optimum, t, multiParamStepContainer] = NewtonParamOptimization(0.5, 0.9, 0, 0, 0, 0, 0.02 ,1, multiParamStepContainer, 9, handles);
...
%Threshold Distance
%EF1
[optimum, t, multiParamStepContainer] = NewtonParamOptimization(0.3, 0.7, 0, 0, 0, 0, 0.02 ,t+1, multiParamStepContainer, 1, handles);
optionHandler = optimum;
handles.OptionHandler = optionHandler;
guidata(handles.figure1, handles);
[optimum, t, multiParamStepContainer] = NewtonParamOptimization(0.1, optionHandler.EdgeFillNucleusAreaWithinNeurite, 0, 0, 0, 0, 0.02 ,t+1, multiParamStepContainer, 2, handles);
optionHandler = optimum;
handles.OptionHandler = optionHandler;
guidata(handles.figure1, handles);

%FuzzFilter
[optimum, t, multiParamStepContainer] = NewtonParamOptimization(0, 1, 0, 0, 0, 0, 1 ,t+1, multiParamStepContainer, 10, handles);
optionHandler = optimum;
handles.OptionHandler = optionHandler;
guidata(handles.figure1, handles);
...
[optimum, t, multiParamStepContainer] = NewtonParamOptimization(20, 70, 0, 0, 0, 0, 3 ,t+1, multiParamStepContainer, 3, handles);
optionHandler = optimum;
handles.OptionHandler = optionHandler;
guidata(handles.figure1, handles);

%Distance from Endpoint
[optimum, t, multiParamStepContainer] = NewtonParamOptimization(10, 60, 0, 0, 0, 0, 3 ,t+1, multiParamStepContainer, 4, handles);
imageHandler.MultiParamStepContainer = multiParamStepContainer;
handles.ImageHandler = imageHandler;
optionHandler = optimum;
%Tolerance Angle
[optimum, t, multiParamStepContainer] = NewtonParamOptimization(10, 60, 0, 0, 0, 0, 3 ,t+1, multiParamStepContainer, 5, handles);
imageHandler.MultiParamStepContainer = multiParamStepContainer;
handles.ImageHandler = imageHandler;
optionHandler = optimum;
...
[optimum, t, multiParamStepContainer] = NewtonParamOptimization(1, 50, 0, 0, 0, 0, 3 ,t+1, multiParamStepContainer, 6, handles);
optionHandler = optimum;
handles.OptionHandler = optionHandler;
guidata(handles.figure1, handles);
...
[optimum, t, multiParamStepContainer] = NewtonParamOptimization(1, 50, 0, 0, 0, 0, 3 ,t+1, multiParamStepContainer, 7, handles);
optionHandler = optimum;
handles.OptionHandler = optionHandler;
guidata(handles.figure1, handles);

%MinSizeNeuriteArea = 2000;
[optimum, t, multiParamStepContainer] = NewtonParamOptimization(500, 8000, 0, 0, 0, 0, 250 ,t+1, multiParamStepContainer, 8, handles);
optionHandler = optimum;
handles.OptionHandler = optionHandler;
guidata(handles.figure1, handles);
...
if(startRate == 0)
    ...
    elseif(FPRatioPercent < 0.15)
        startRate = TPRatioPercent - (FPRatioPercent/6);
    elseif(FPRatioPercent < 0.2)
        startRate = TPRatioPercent - (FPRatioPercent/3);
    else
...
if(stopRate == 0)
    ...
    elseif(FPRatioPercent < 0.15)
         stopRate = TPRatioPercent - (FPRatioPercent/6);
    elseif(FPRatioPercent < 0.2)
        stopRate = TPRatioPercent - (FPRatioPercent/3);
    else
...
handles = guidata(handles.figure1);
LoadImageFolder(foldernameConverted, handles); 
%Call 'Load Cellomics CSV'
LoadCSV(['/' expname '-Kernkoordinaten.csv'],foldername,handles);
%Call Load Neuron CSV
LoadNeuronCSV(['/' expname '.csv'],foldername,handles);
ConvertPicturesTo8Bit(handles);
