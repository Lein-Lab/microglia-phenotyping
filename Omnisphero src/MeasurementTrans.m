...
function [summary Summary2 Summary3 Summary4 Summary5, ExpName, csvHandler] = MeasurementTrans ( foldername, csvHandler, neuronHandler)
...
for(j=1:numel(allfilesOrig))
    % Check of parent folder contains png images
...
for(i=1:numel(allfiles))
    ...
    ind = strfind([foldername2 '/ConvertedCellomics' allfiles(i).name],'Skeleton');
    if(numel(ind) > 0)%
         n = n + 1
         mm = n + 1;
         % Read images
         ...
         newFileNeuronNuc = regexprep(allfiles(i).name,'Skeleton','NucleusBigWatershed','ignorecase');
         ...
         % nuclei!
        NucleusM = sparse(zeros(size(binaryimgNeurite)));
        NeuronNuc = logical(NeuronNuc);
        if(sum(sum(binaryimgNeurite)) >0)
        % If yes determine number of particles
        sR = regionprops(NeuronNuc, 'Centroid');
        sRdimensions = size(sR);
        sRdimensions = sRdimensions(1);
        else
        ...
        % We need to tajke the original image, since otherwise branching
        ...
        newFileEndBranch = regexprep(allfiles(i).name,'Skeleton','OligoSmall','ignorecase');
        newFileEndBranch2 = regexprep(allfiles(i).name,'Skeleton','OligoBig','ignorecase');
        ...
        imwrite(SkeletonWoBranchWoNuc,[foldername2 '/' allfiles(i).name]);
       ...
       for(h=1:NumberofNeurons)
           ...
           % Cretae header for all summaries
            ...
            Summary5{1,1} = 'ImageName';
            Summary5{1,2} = 'TotalLength';
            Summary5{1,3} = 'NumberofTips';
            Summary5{1,4} = 'NumberofBranches';
       ...
       % Save ring mask     
       newfileRing = regexprep(allfiles(i).name,'Skeleton','AstroBig','ignorecase');
       newfileRing2 = regexprep(allfiles(i).name,'Skeleton','AstroSmall','ignorecase');
