...
classdef CSVCoordinates < handle
    ...
    properties
        ...
        CellPosMap;
        %Cellomics Positions
        CellPosMatrix = containers.Map();
        Well96Offsets;
        Well96Offsets484;
        Well96Offsets16Bit;
        Well96Offsets16Bit255;
        Well48Offsets;
        Well48Offsets16Bit;
        WellWholeOTOffsets;
        ...
        OligoPositionsEdgeComposite;
        OmniNucleusPositions;
        
        
        NeuronPositionsEdgeFill = containers.Map();
        ...
        NeuronPositionsSkelDeleted = containers.Map();
        NeuronPositionsSkeletonization = containers.Map();
    ...
    methods (Access = public)
        ...
        function e = CSVCoordinates()
            ...
            self.ManualNeuronPositions = containers.Map();
            currentPos = [1,1];
        ...
        %Fill Offsets for 96er in 16 Bit pattern with 255 images but only
        %196 scanned! Achtung Schnecke wir von auﬂen nach innen
        %zusammengesetzt!
            structureMatrix96255 = zeros(14,14);
            currentPos = [14,1];
            direction = Direction.Left;
            for i=196:-1:1
                e.Well96Offsets16Bit255{i} = {e.OffsetSize * (currentPos(1)-1), e.OffsetSize * (currentPos(2)-1)};         
                structureMatrix96255(currentPos(1),currentPos(2)) = 1;
                if(direction == Direction.Right)
                    if(currentPos(1) + 1 <= 14 && structureMatrix96255(currentPos(1)+1, currentPos(2)) == 0)
                        currentPos(1) = currentPos(1) + 1;
                    else
                        currentPos(2) = currentPos(2) - 1;
                        direction = Direction.Down;
                ...
                elseif(direction == Direction.Down)
                    if(currentPos(2) - 1 >= 1 && structureMatrix96255(currentPos(1), currentPos(2)-1) == 0)
                        currentPos(2) = currentPos(2) - 1;
                    else
                        currentPos(1) = currentPos(1) - 1;
                        direction = Direction.Left;
                ...
                elseif(direction == Direction.Left)
                    if(currentPos(1) - 1 >= 1 && structureMatrix96255(currentPos(1)-1, currentPos(2)) == 0)
                        currentPos(1) = currentPos(1) - 1;
                    else
                        currentPos(2) = currentPos(2) + 1;
                        direction = Direction.Up;
                ...
                elseif(direction == Direction.Up)
                    if(currentPos(2) + 1 <= 14 && structureMatrix96255(currentPos(1), currentPos(2)+1) == 0)
                        currentPos(2) = currentPos(2) + 1;
                    else
                        currentPos(1) = currentPos(1) + 1;
                        direction = Direction.Right;
        ...
        function i=GetPictureNumberFromPosition(self, xPos, yPos, wellType)
            ...
            elseif(wellType == WellType.Well9616255)
                 dict = self.Well96Offsets16Bit255;
       ...
       function [x y] = CalculateGlobalCoordinatesCSV(self, field, leftIndex, topIndex, wellType)
           %Formel fÔøΩr richtige Position:
           % Sqrt(ObjectArea) = KantenlÔøΩnge
           % Versatz = Sqrt(KantenlÔøΩnge^2+KantenlÔøΩnge^2) / 2
           % y = y-Versatz
           % x = x+Versatz
           ...
           elseif(wellType == WellType.Well9616)
                ...
                y=self.Well96Offsets{field}{2} + (512 - topIndex) + 1;
           elseif(wellType == WellType.Well9616255)
                x=self.Well96Offsets16Bit255{field}{1} + leftIndex + 1;
                y=self.Well96Offsets16Bit255{field}{2} + (topIndex) + 1;     
           elseif(wellType == WellType.Well96484)
        ...
        function [x y] = CalculateGlobalCoordinatesImage(self, field, leftIndex, topIndex, wellType)
           ...
           elseif(wellType == WellType.Well9616255)
                x=self.Well96Offsets16Bit255{field}{1} + leftIndex;
                y=self.Well96Offsets16Bit255{field}{2} + (topIndex);     
           elseif(wellType == WellType.Well96484)
        ...
        function ReadMDBFile(self, datafile, wellType, eventTypeProfileFilter)            
            self.CellPosMatrix = containers.Map(); 
            self.ManualNeuronPositionsSparse = containers.Map();
            headerMap = containers.Map();
            con = AccessDatabase_OLEDB(datafile);            
            
            sqlString = 'SELECT c.TOP, c.LEFT, c.WFieldID, f.FileName, f.wCol, c.ID FROM CELL c INNER JOIN WFIELD f on c.WFieldID = f.ID';
            dbResult = con.query(sqlString);            
            nCells = size(dbResult,1);
            for(i=1:nCells)
                cellID = dbResult(i,1);
                cellID = cellID{1};
                wellNameRegexp = strcat('ARRAY.*\d*_(\D\d+).*');
                wellDB = dbResult(i,4);
                wellDB = wellDB{1};                
                tokensc1 = regexpi(wellDB, wellNameRegexp, 'tokens');
                currentWellName = tokensc1{1};
                ...
                %XCentroid oder Left im CSV
                leftIndexCell = dbResult(i,2);                
                leftIndexCell = leftIndexCell{1};
                
                %Top oder YCentroid
                topIndexCell = dbResult(i,1);
                topIndexCell = topIndexCell{1};
                %Field
                fieldIndexCell = dbResult(i,5);
                fieldIndexCell = fieldIndexCell{1};
                lefIndexCell = leftIndexCell + 1;
                topIndexCell = topIndexCell + 1;           
                filteredOut=0;
                if(eventTypeProfileFilter == 1)
                    selectEventTyp = strcat('SELECT valdbl FROM CellFeature WHERE CellID = ',cellID,' AND TypeID = 17830');
                    cellPosAttributes = containers.Map();
                    eventTypeProfile = con.query(selectEventTyp);
                    eventTypeProfile = eventTypeProfile{1};                     
            ...
            if exist(datafile, 'file') > 0
                ...
                while (~feof(filePointer))
                    ...
                    if(allMatch == 1)
                        ...
                        currentMatrix = self.CellPosMatrix(currentWellName);
                        if(y > size(currentMatrix,1))
                            y=size(currentMatrix,1);
                        end
                        if(x > size(currentMatrix,2))
                            x=size(currentMatrix,2);
                        end
        ...
        function [nucCol nucRow euclidDist] = FindNucleusForNeuron(self, neuCol, neuRow, NucleusM, maxDist, rVec, tolerance)
           nucCol = -1;
           nucRow = -1;
           found = 0;
           iterator=0;
        ...
        %Get For NeuronPosition col, row nearest Nucleus in NucleusM
        function [nucCol nucRow euclidDist] = FindNucleusForNeuronWithoutDirection(self, neuCol, neuRow, NucleusM, D, Ind,sizeY,sizeX)
           nucCol = -1;
           nucRow = -1;
           found = 0;
           iterator=0;
